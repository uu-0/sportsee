...
function Activity({ useMockData }) {
  ...
  useEffect(() => {
    if (useMockData) {
      ...
      if (activity) {
        const limitedData = activity.sessions.slice(-10);
        setActivityData(limitedData);
      } else {
        console.error('Activité de l\'utilisateur non trouvée');
      }
    } else {
      //version finale avec l'API
      fetchUserActivity(userId)
        .then(data => {
          const limitedData = data.sessions.slice(-10);
          setActivityData(limitedData);
        })
        .catch(error => console.error('Erreur lors de la récupération des données:', error));
    }
  }, [userId, useMockData]); //useEffect est déclenché chaque fois que userId ou useMockData change
